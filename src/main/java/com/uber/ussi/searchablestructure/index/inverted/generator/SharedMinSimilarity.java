...
/**
 * The highest minimum similarity any of a search's shards has proved the answer will hold, shared
 * between them.
 *
 ...
 * kept score is at least that shard's, and no row scoring below it can reach the answer. One
 * shard's minimum similarity is therefore sound for every shard, and this holds the highest any of
 * them has published.
 *
 * <p>A shard whose heap is not yet full has proved nothing and publishes nothing.
 *
 * <p>This only rises, and it is one value rather than a structure, so it needs no lock. Shards keep
