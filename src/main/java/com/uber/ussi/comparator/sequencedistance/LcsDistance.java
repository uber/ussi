 ...
 * other, complementing the longest common subsequence (LCS): every element outside the LCS has
 * to be deleted from one sequence or inserted into the other, so this distance is
 * {@code length1 + length2 - 2 * lcsLength}.
 *
 * <p>Without substitution, rewriting an element costs a deletion and an insertion, so this is
 * never below the Levenshtein distance; in exchange each edit moves exactly one element into or
