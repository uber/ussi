...
public final class SignatureIndex extends BaseInvertedIndex {
  ...
  public SignatureIndex(
    ...
    super(namespaceConfig, rowNumToTermsAndValuesMap, rowNumToMetaMap, KeyType.SIGNATURE);
  }
  ...
  protected KeyAndUniTransformedValue[] getSparseKeysAndUniTransformedValues(
    ...
    KeyAndUniTransformedValue[] sparseKeysAndValues =
        new KeyAndUniTransformedValue[sparseKeys.length];
    ...
    for (int i = 0; i < sparseKeys.length; ++i) {
      sparseKeysAndValues[i] =
          new KeyAndUniTransformedValue(sparseKeys[i], signatureUniTransformedValue);
    }
